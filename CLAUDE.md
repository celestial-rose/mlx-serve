...
- **A media placeholder id occurs in ordinary TEXT**: media boundaries are gated on the request CARRYING media (`firstMediaPlaceholder(has_media)`). Select active-turn media from WIRE METADATA before decoding (`activeWireMediaIndex`).
- **A tool-response turn can render under the USER marker** — `resolvedUserMarkersAfter` reads the RENDERED prompt. A content array's text parts JOIN in order (`joinedTextParts`, #195).
...
- **Tokenizer-derived config is set in ONE place for both load paths** (`ModelConfig.applyTokenizer`: EOS merges, user-turn marker, LFM2 image tokens): the registry path lacked the marker, so every image on an app-loaded model landed at the prompt's END (`userTurnInsertPos` fallback) and the model saw it "attached again" each turn.
...
- **An f32 SCALAR promotes every bf16 operand** — scalars via `scalarOf(v, dtype)`; a load-time const table in the wrong dtype widens every read (`constTableAs`); a chain that returns f32 by design makes the CALLER own the dtype. Tell: `[dtype-trace] residual widened`.
...
- **A verify lane is never byte-identical to stock**; parity = fp32-dequant truth per width, RMS ratio vs stock ≤ 3.0x, never cosine (`VerifyQmmParity`). 2-bit GEMV accumulates in f32 from exact products (`qmv2.zig`, M 1..3; past 3 stock serves).
